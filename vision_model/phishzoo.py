...
def get_first_file(dir):
    if not os.path.exists(dir):
        return None
    lst = [os.path.join(dir, f) for f in os.listdir(dir) if os.path.isfile(os.path.join(dir, f))]
    if len(lst) == 0:
        return None
    return lst[0]
...
def main(data_folder, mode, outfile, target_root):
    ...
    for subfolder in tqdm(os.listdir(data_folder)):
        file_data_path = data_folder + '/' + subfolder + "/URL"
        file_path = get_first_file(file_data_path)
        html_path = data_folder + '/' + subfolder + "/RAW-HTML"
        html_path = get_first_file(html_path)
        if html_path == None or file_path == None:
            continue
        data_path.append(data_folder + '/' + subfolder)
        if mode == 'phish':
            f = open(file_path, 'r', encoding="ISO-8859-1").read()
            # print(f)
            # f = eval(f)
            urls.append(f)
        else:
        ...
        for folder in os.listdir(target_root):
            ...
            for token in web_token.split():
                if token in ground_token: # trigger SIFT when at least 1 token match
                    ...
                    temp = data_path[i] + '/SCREEN-SHOT'
                    screenshot = get_first_file(temp)
                    check = SIFT(screenshot, target_root + '/' + folder)
                    _, _, sim = check.match()
